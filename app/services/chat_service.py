...
SYSTEM_PROMPT = (
    ...
    "call create_payment_order so the inline UPI QR payment card appears directly in the "
    ...
    "show a payment option is the UPI QR card the system renders automatically "
    "after a successful create_payment_order call. After calling create_payment_order "
    "successfully, just say in plain text (no links) that a UPI QR code has "
    "appeared below for that total amount (UPI ID qxl-diagnostics@pingpay) and ask them to scan and pay. Do not claim "
    "the payment succeeded yourself — wait until they confirm. ONLY if create_payment_order itself fails with an error should "
