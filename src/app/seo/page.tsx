            ...
            {filtered.map((item) => (
              <div key={item.id} className="pt-6 first:pt-0 flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div className="space-y-2 flex-1 max-w-3xl">
                  <span className="bg-sky-50 text-sky-700 dark:bg-sky-950/20 dark:text-sky-400 px-2.5 py-0.5 rounded-md text-[10px] font-extrabold uppercase tracking-wider">
                    {item.page}
                  </span>
      ...
      {editingItem && (
        <div className="fixed inset-0 bg-black/55 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-gray-900 rounded-2xl max-w-lg w-full shadow-2xl border border-gray-150 dark:border-gray-800 overflow-hidden">
            ...
            {/* AI SEO Assistant */}
            <div className="px-6 py-3 bg-sky-50/50 dark:bg-sky-950/10 border-b border-sky-100/50 dark:border-sky-900/10 flex items-center justify-between gap-4">
              ...
              <button 
                ...
                className="px-3 py-1 bg-sky-600 text-white rounded-lg text-[10px] font-black uppercase tracking-wider flex items-center gap-1 shadow-xs cursor-pointer"
              >
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-gray-100 dark:border-gray-800">
                ...
                <button 
                  type="submit" 
                  className="px-5 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-lg text-xs font-medium shadow-sm cursor-pointer"
                >
                  Save Tags
                </button>
              </div>
