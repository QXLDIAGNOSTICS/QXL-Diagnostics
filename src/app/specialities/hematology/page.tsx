...
export default function HematologyPage() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      {/* Hero Banner */}
      <section className="bg-gradient-to-r from-red-700 to-red-950 text-white py-16 lg:py-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl">
            <span className="inline-block bg-white/20 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-widest mb-4">Hematology</span>
            <h1 className="text-3xl md:text-5xl font-bold mb-4 text-white leading-tight">Advanced Blood &amp; Coagulation Diagnostics in Bengaluru</h1>
            ...
            <div className="flex flex-wrap gap-3">
              <a href="#test-profiles" className="bg-white text-red-800 font-bold px-5 py-2.5 rounded-full hover:bg-red-50 transition-colors text-sm">View Test Profiles</a>
              <Link href="/book" className="bg-red-500 border-2 border-white text-white font-bold px-5 py-2.5 rounded-full hover:bg-red-400 transition-colors text-sm">Book a Test</Link>
      ...
      <div className="container mx-auto px-4 py-3">
        <div className="flex items-center text-xs text-gray-500 flex-wrap gap-1">
          ...
          <Link href="/specialities" className="hover:text-red-600">Our Specialities</Link>
          <ChevronRight className="w-3 h-3" />
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            {/* Who Should Take */}
            <section className="bg-red-50 border border-red-150 rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-4">
                <Users className="w-5 h-5 text-red-700" />
                <h2 className="text-lg font-bold text-red-950">Who Should Take These Tests?</h2>
              </div>
              <ul className="space-y-2 text-sm text-red-900">
                {[
                  ...
                  <li key={i} className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-red-600 mt-0.5 flex-shrink-0" />{item}</li>
                ))}
              </ul>
            </section>
            ...
            <section id="test-profiles">
              <h2 className="text-2xl font-bold text-gray-900 mb-5 flex items-center gap-2">
                <Droplet className="w-6 h-6 text-red-600" /> Hematology Test Profiles
              </h2>
              <div className="grid gap-5">
                {[
                  ...
                  <div key={t.name} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm hover:border-red-400 hover:shadow-md transition-all">
                    <div className="flex flex-col md:flex-row justify-between gap-3">
                      ...
                      <Link href="/book" className="self-start bg-red-600 text-white font-semibold px-4 py-2 rounded-lg whitespace-nowrap hover:bg-red-700 transition-colors text-sm">Book Now</Link>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section className="bg-white border border-gray-200 rounded-2xl p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2"><Stethoscope className="w-5 h-5 text-red-600" /> For Clinicians</h2>
            ...
            {/* Why QXL */}
            <section className="bg-gradient-to-br from-red-50 to-white border border-red-100 rounded-2xl p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-5">Why Choose QXL Diagnostics?</h2>
              <div className="grid sm:grid-cols-2 gap-4">
                {[
                  { icon: <Shield className="w-5 h-5 text-red-600" />, title: "NABL Certified Excellence", desc: "Rigorous internal and external quality control programs ensuring reliable blood metrics." },
                  { icon: <Microscope className="w-5 h-5 text-red-600" />, title: "Consultant Pathologist Verification", desc: "Every blood smear flag is manually reviewed under microscopy by our experts." },
                  { icon: <Activity className="w-5 h-5 text-red-600" />, title: "HPLC Gold Standard Testing", desc: "High-resolution liquid chromatography for accurate thalassemia trait mapping." },
                  { icon: <CheckCircle className="w-5 h-5 text-red-600" />, title: "Safe & Painless Extraction", desc: "Certified phlebotomists trained in pediatric and geriatric blood draws at home." }
                ].map((item, i) => (
                  <div key={i} className="flex gap-3">
                    <div className="mt-0.5 bg-red-100 p-1.5 rounded-lg flex-shrink-0">{item.icon}</div>
                    <div>
            ...
            {/* CTA Banner */}
            <section className="bg-gradient-to-r from-red-700 to-red-900 rounded-2xl p-6 text-white">
              <h3 className="text-lg font-bold mb-2">Book Your Hematology Test Today</h3>
              <p className="text-red-100 text-sm mb-4">Fast same-day digital reports. Safe home collection available across Bengaluru.</p>
              <div className="flex flex-wrap gap-3">
                <Link href="/book" className="bg-white text-red-700 font-bold px-5 py-2 rounded-full text-sm hover:bg-red-50 transition-colors">Book a Test</Link>
          ...
          <div className="lg:col-span-1 space-y-5">
            <div className="bg-gradient-to-br from-red-700 to-red-900 text-white rounded-2xl p-5 shadow-lg">
              <h3 className="text-lg font-bold mb-2 text-white">Need help choosing the right test?</h3>
              <p className="text-red-100 text-xs mb-4">Our medical consultant team can guide you to the right diagnostic profile for your symptoms.</p>
              <a href="tel:+919964639639" className="w-full bg-white font-bold py-2.5 rounded-xl flex justify-center hover:bg-red-50 transition-colors mb-2 shadow text-sm" style={{ color: '#be123c' }}>📞 Call +91 99646 39639</a>
              <a href="https://api.whatsapp.com/send?phone=919964639639" target="_blank" rel="noreferrer" className="w-full border border-red-400 bg-red-800 text-white font-bold py-2.5 rounded-xl flex justify-center hover:bg-red-700 transition-colors text-sm">WhatsApp Us</a>
              <Link href="/upload-prescription" className="mt-2 w-full border border-red-400 bg-transparent text-white font-bold py-2.5 rounded-xl flex justify-center hover:bg-white/10 transition-colors text-sm"><FileText className="w-4 h-4 mr-2" /> Upload Prescription</Link>
            </div>
            <div className="bg-white border border-gray-200 rounded-2xl p-5">
              ...
              <ul className="space-y-2">
                {SPECIALITIES.map(spec => (
                  <li key={spec}>
                    <Link href={`/specialities/${spec.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`} className="text-gray-600 hover:text-red-600 text-sm flex items-center justify-between group font-medium">
                      {spec}<ChevronRight className="w-3.5 h-3.5 text-gray-300 group-hover:text-red-400" />
                    </Link>
                  </li>
                ))}
              </ul>
