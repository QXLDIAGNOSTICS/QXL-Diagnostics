...
export default function InfectiousDiseasesPage() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      {/* Hero Banner */}
      <section className="bg-gradient-to-r from-teal-700 to-teal-950 text-white py-16 lg:py-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl">
            ...
            <h1 className="text-3xl md:text-5xl font-bold mb-4 text-white leading-tight">Advanced serology &amp; Molecular PCR Testing in Bengaluru</h1>
            <p className="text-teal-100 text-base md:text-lg mb-8 leading-relaxed">
            ...
            <div className="flex flex-wrap gap-3">
              <a href="#test-profiles" className="bg-white text-teal-850 font-bold px-5 py-2.5 rounded-full hover:bg-teal-50 transition-colors text-sm" style={{ color: '#0f766e' }}>View Test Profiles</a>
              <Link href="/book" className="bg-teal-500 border-2 border-white text-white font-bold px-5 py-2.5 rounded-full hover:bg-teal-400 transition-colors text-sm">Book a Test</Link>
      ...
      <div className="container mx-auto px-4 py-3">
        <div className="flex items-center text-xs text-gray-500 flex-wrap gap-1">
          ...
          <Link href="/specialities" className="hover:text-teal-600">Our Specialities</Link>
          <ChevronRight className="w-3 h-3" />
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            {/* Who Should Take */}
            <section className="bg-teal-50 border border-teal-150 rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-4">
                <Users className="w-5 h-5 text-teal-700" />
                <h2 className="text-lg font-bold text-teal-950">Who Should Take These Tests?</h2>
              </div>
              <ul className="space-y-2 text-sm text-teal-900">
                {[
                  ...
                  <li key={i} className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-teal-600 mt-0.5 flex-shrink-0" />{item}</li>
                ))}
              </ul>
            </section>
            ...
            <section id="test-profiles">
              <h2 className="text-2xl font-bold text-gray-900 mb-5 flex items-center gap-2">
                <Microscope className="w-6 h-6 text-teal-600" /> Infectious Diseases Test Profiles
              </h2>
              <div className="grid gap-5">
                {[
                  ...
                  <div key={t.name} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm hover:border-teal-400 hover:shadow-md transition-all">
                    <div className="flex flex-col md:flex-row justify-between gap-3">
                      ...
                      <Link href="/book" className="self-start bg-teal-600 text-white font-semibold px-4 py-2 rounded-lg whitespace-nowrap hover:bg-teal-700 transition-colors text-sm">Book Now</Link>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section className="bg-white border border-gray-200 rounded-2xl p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2"><Stethoscope className="w-5 h-5 text-teal-600" /> For Clinicians</h2>
            ...
            {/* Why QXL */}
            <section className="bg-gradient-to-br from-teal-50 to-white border border-teal-100 rounded-2xl p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-5">Why Choose QXL Diagnostics?</h2>
              <div className="grid sm:grid-cols-2 gap-4">
                {[
                  { icon: <Shield className="w-5 h-5 text-teal-600" />, title: "NABL Accredited Quality", desc: "Rigorous quality controls and inter-laboratory testing ensuring highest accuracy." },
                  { icon: <Microscope className="w-5 h-5 text-teal-600" />, title: "Molecular PCR Capabilities", desc: "Equipped with state-of-the-art thermocyclers for fast diagnostic assays." },
                  { icon: <Activity className="w-5 h-5 text-teal-600" />, title: "Antimicrobial Guidance", desc: "Susceptibility reports utilize current clinical standards for prescribing." },
                  { icon: <CheckCircle className="w-5 h-5 text-teal-600" />, title: "Free Home Collection", desc: "Professional phlebotomists trained in sterile sample collection protocols." }
                ].map((item, i) => (
                  <div key={i} className="flex gap-3">
                    <div className="mt-0.5 bg-teal-100 p-1.5 rounded-lg flex-shrink-0">{item.icon}</div>
                    <div>
            ...
            {/* CTA Banner */}
            <section className="bg-gradient-to-r from-teal-700 to-teal-900 rounded-2xl p-6 text-white">
              <h3 className="text-lg font-bold mb-2">Book Your Infectious Disease Panel</h3>
              <p className="text-teal-100 text-sm mb-4">Same-day reports for routine serology and molecular tests. Home collection available across Bengaluru.</p>
              <div className="flex flex-wrap gap-3">
                <Link href="/book" className="bg-white text-teal-700 font-bold px-5 py-2 rounded-full text-sm hover:bg-teal-50 transition-colors">Book a Test</Link>
          ...
          <div className="lg:col-span-1 space-y-5">
            <div className="bg-gradient-to-br from-teal-700 to-teal-900 text-white rounded-2xl p-5 shadow-lg">
              <h3 className="text-lg font-bold mb-2 text-white">Need help choosing the right test?</h3>
              <p className="text-teal-100 text-xs mb-4">Our medical consultant team can guide you to the right diagnostic profile for your symptoms.</p>
              <a href="tel:+919964639639" className="w-full bg-white font-bold py-2.5 rounded-xl flex justify-center hover:bg-teal-50 transition-colors mb-2 shadow text-sm" style={{ color: '#0f766e' }}>📞 Call +91 99646 39639</a>
              <a href="https://api.whatsapp.com/send?phone=919964639639" target="_blank" rel="noreferrer" className="w-full border border-teal-400 bg-teal-800 text-white font-bold py-2.5 rounded-xl flex justify-center hover:bg-teal-750 transition-colors text-sm">WhatsApp Us</a>
              <Link href="/upload-prescription" className="mt-2 w-full border border-teal-400 bg-transparent text-white font-bold py-2.5 rounded-xl flex justify-center hover:bg-white/10 transition-colors text-sm"><FileText className="w-4 h-4 mr-2" /> Upload Prescription</Link>
            </div>
            <div className="bg-white border border-gray-200 rounded-2xl p-5">
              ...
              <ul className="space-y-2">
                {SPECIALITIES.map(spec => (
                  <li key={spec}>
                    <Link href={`/specialities/${spec.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`} className="text-gray-600 hover:text-teal-600 text-sm flex items-center justify-between group font-medium">
                      {spec}<ChevronRight className="w-3.5 h-3.5 text-gray-300 group-hover:text-teal-400" />
                    </Link>
                  </li>
                ))}
              </ul>
