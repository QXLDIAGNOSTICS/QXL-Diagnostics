...
export default function NeurologyPage() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <section className="bg-gradient-to-r from-indigo-700 to-indigo-900 text-white py-16 lg:py-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl">
            ...
            <h1 className="text-3xl md:text-5xl font-bold mb-4 text-white leading-tight">Advanced Neurology &amp; Autoimmune Neurology Testing in Bengaluru</h1>
            ...
            <div className="flex flex-wrap gap-3">
              <a href="#test-profiles" className="bg-white text-indigo-700 font-bold px-5 py-2.5 rounded-full hover:bg-indigo-50 transition-colors text-sm">View Test Profiles</a>
              <Link href="/book" className="bg-indigo-500 border-2 border-white text-white font-bold px-5 py-2.5 rounded-full hover:bg-indigo-400 transition-colors text-sm">Book a Test</Link>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-10">
            ...
            <section id="test-profiles">
              <h2 className="text-2xl font-bold text-gray-900 mb-5 flex items-center gap-2">
                <Brain className="w-6 h-6 text-indigo-600" /> Neurology Test Profiles
              </h2>
              <div className="grid gap-5">
                {[
                  ...
                  <div key={t.name} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm hover:border-indigo-400 hover:shadow-md transition-all">
                    <div className="flex flex-col md:flex-row justify-between gap-3">
                      ...
                      <Link href="/book" className="self-start bg-indigo-600 text-white font-semibold px-4 py-2 rounded-lg whitespace-nowrap hover:bg-indigo-700 transition-colors text-sm">Book Now</Link>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section className="bg-white border border-gray-200 rounded-2xl p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-3 flex items-center gap-2"><Stethoscope className="w-5 h-5 text-indigo-600" /> For Clinicians</h2>
            ...
            <section className="bg-gradient-to-br from-indigo-50 to-white border border-indigo-100 rounded-2xl p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-5">Why Choose QXL Diagnostics?</h2>
              <div className="grid sm:grid-cols-2 gap-4">
                {[
                  { icon: <Shield className="w-5 h-5 text-indigo-600" />, title: "NABL Certified Laboratory", desc: "ISO 15189 accredited testing for super speciality panels." },
                  { icon: <Microscope className="w-5 h-5 text-indigo-600" />, title: "Expert-Reviewed Reports", desc: "Consultant neurologist-supported reporting and interpretation notes." },
                  { icon: <Activity className="w-5 h-5 text-indigo-600" />, title: "Advanced Molecular Platforms", desc: "FilmArray PCR, CBA, ELISA for autoimmune and neuro-infectious testing." },
                  { icon: <CheckCircle className="w-5 h-5 text-indigo-600" />, title: "Urgent Priority Processing", desc: "Emergency CSF/autoimmune panel processing available on doctor request." },
                ].map((item, i) => (
                  <div key={i} className="flex gap-3">
                    <div className="mt-0.5 bg-indigo-100 p-1.5 rounded-lg flex-shrink-0">{item.icon}</div>
                    <div>
            ...
            <section className="bg-gradient-to-r from-indigo-700 to-indigo-900 rounded-2xl p-6 text-white">
              <h3 className="text-lg font-bold mb-2">Book Your Neurology Profile Today</h3>
              <p className="text-indigo-100 text-sm mb-4">Home collection available. Doctor referral required for CSF-based tests. Reports reviewed by consultant pathologist.</p>
              <div className="flex flex-wrap gap-3">
                <Link href="/book" className="bg-white text-indigo-700 font-bold px-5 py-2 rounded-full text-sm hover:bg-indigo-50">Book a Test</Link>
          ...
          <div className="lg:col-span-1 space-y-5">
            <div className="bg-gradient-to-br from-indigo-700 to-indigo-900 text-white rounded-2xl p-5 shadow-lg">
              <h3 className="text-lg font-bold mb-2 text-white">Need help with neurology testing?</h3>
              <p className="text-indigo-100 text-xs mb-4">Our consultants are available for guidance on panel selection and result interpretation.</p>
              <a href="tel:+919964639639" className="w-full bg-white font-bold py-2.5 rounded-xl flex justify-center hover:bg-indigo-50 transition-colors mb-2 shadow text-sm" style={{ color: '#3730a3' }}>📞 Call +91 99646 39639</a>
              <a href="https://api.whatsapp.com/send?phone=919964639639" target="_blank" rel="noreferrer" className="w-full border border-indigo-400 bg-indigo-800 text-white font-bold py-2.5 rounded-xl flex justify-center hover:bg-indigo-700 transition-colors text-sm">WhatsApp Us</a>
              <Link href="/upload-prescription" className="mt-2 w-full border border-indigo-400 text-white font-bold py-2.5 rounded-xl flex justify-center hover:bg-white/10 transition-colors text-sm"><FileText className="w-4 h-4 mr-2" /> Upload Prescription</Link>
            </div>
            <div className="bg-white border border-gray-200 rounded-2xl p-5">
              ...
              <ul className="space-y-2">
                {SPECIALITIES.map(spec => (
                  <li key={spec}>
                    <Link href={`/specialities/${spec.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`} className="text-gray-600 hover:text-indigo-600 text-sm flex items-center justify-between group font-medium">
                      {spec}<ChevronRight className="w-3.5 h-3.5 text-gray-300 group-hover:text-indigo-400" />
                    </Link>
                  </li>
                ))}
              </ul>
